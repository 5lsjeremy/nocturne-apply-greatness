...
{
    public sealed class AssignmentMetadataOverlay : IVectorOverlay
    {
        public string Id => "student.assignment.metadata";

        public void Apply(IResolvedVectorContext ctx, object? input)
        {
            var typed = (StudentAnalyzeInput)input!;

            var topic = ExtractTopic(typed.UnitSlug);
            var difficulty = EstimateDifficulty(typed.AssignmentText);

            ctx.ClusterMetadata["topic"] =
                Slug.Create("topic", topic);

            ctx.ClusterMetadata["difficulty"] =
                Slug.Create("difficulty", difficulty);

            ctx.ClusterMetadata["skillCategory"] =
                Slug.Create("skill", "analysis");
        }

        private static string ExtractTopic(string slug)
        {
            var parts = slug.Split('.');
            return parts.Length > 2 ? parts[2] : slug;
        }

        private static string EstimateDifficulty(string text)
        {
            text = text.ToLowerInvariant();

            if (text.Contains("explain") || text.Contains("analyze"))
                return "medium";

            if (text.Contains("evaluate") || text.Contains("justify"))
                return "high";
