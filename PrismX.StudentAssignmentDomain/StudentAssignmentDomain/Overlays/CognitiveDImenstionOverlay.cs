...
{
    public sealed class CognitiveDimensionOverlay : IVectorOverlay
    {
        public string Id => "student.cognitive";

        public void Apply(IResolvedVectorContext ctx, object? input)
        {
            var typed = (StudentAnalyzeInput)input!;
            var dimension = CognitiveDimensionMapper.Map(typed);

            ctx.ClusterMetadata["cognitive"] = dimension;
        }
