...
{
    public sealed class DisabilityOverlay : IVectorOverlay
    {
        public string Id => "student.disability";

        public void Apply(IResolvedVectorContext ctx, object? input)
        {
            var typed = (StudentAnalyzeInput)input!;
            ctx.ClusterMetadata["disability"] = DisabilityProfileMapper.Map(typed);
        }
