...
{
    public sealed class StudentProjectionOverlay : IVectorOverlay
    {
        public string Id => "student.projection";

        public void Apply(IResolvedVectorContext ctx, object? input)
        {
            var typed = (StudentAnalyzeInput)input!;

            ctx.ClusterMetadata["unit"] =
                Slug.Create("unit", typed.UnitSlug);

            ctx.ClusterMetadata["assignment"] =
                Slug.Create("assignment", typed.AssignmentText);

            ctx.ClusterMetadata["response"] =
                Slug.Create("response", typed.StudentResponse);
        }

        // -----------------------------
        // Delegate all math to VectorOverlay
        // -----------------------------
