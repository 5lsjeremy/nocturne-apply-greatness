...
{
    public sealed class DifficultyOverlay : IVectorOverlay
    {
        public string Id => "student.difficulty";

        public void Apply(IResolvedVectorContext ctx, object? input)
        {
            var typed = (StudentAnalyzeInput)input!;
            var difficulty = DifficultyEstimator.Estimate(typed.AssignmentText);
